...
10. 🧪 **456 条单测全绿** —— 纯逻辑一律抽 `*Logic`；构建前置阿里云镜像（原来直连大 jar 仅 ~14 KB/s，会卡死）。
...
| 一卡通余额趋势 | 每次取到余额都在本机记一笔，详情页直接给出**今日 / 近 N 天**的变化（流水拿不到，这是替代方案） |
...
| 空教室查询 | 从「只看默认校区」改为 **校区 → 教学楼 → 教室** 三级折叠（默认校区排最前并自动展开） |
| 地图校区切换 | 原版只有「乡 / 村」两颗硬编码按钮 → 改为按校区表自动生成（**乡 / 村 / 珠 / 嘉**，含珠海与嘉兴校区），切换后弹提示且按钮高亮 |
| 桌面小组件页序 | **课程 / DDL / 动态 / 座位**，页号改存**页名**（重排不会静默错页） |
| 桌面小组件点条目 | 课程 / DDL / 动态条目点了**直接落到对应 tab**（原版无组件） |
...
| 单测 | 少量 | **462 条**（座位 145 / 数据 154 / 组件 69 / 提醒 40 / 设置 36 / 课表 12 / 地图 6） |
