...
## 2026-09-26 v1.9.12 地图页：所有校区一键切换 + 切换提示

- **地图页不再只有「乡 / 村」两颗硬编码按钮**，改成按校区表自动生成，新增**珠海校区**：
  - `MapCampus`（新文件）集中放校区表：校名 / 按钮单字 / 归一化坐标 / 缩放倍率，
    顺序 = 按钮顺序，第一个是进页面的默认定位（仍是良乡）
  - 切换后弹提示 **「已切换到 XX 校区」**，并且**当前校区按钮高亮**——
    地图本身能拖，「点了没动」和「跳走了」在眼里是一样的，必须有反馈
  - `MapScreen` 从无参改成收 `MainController`（只为弹提示），`IndexScreen` 相应传参
- **坐标是怎么定的（可复现）**：瓦片服务是**全球墨卡托**、只提供 z15~z18，
  于是写成换算 `x=(lon+180)/360`、`y=0.5-ln(tan(45°+lat/2))/2π`，
  拿现存的 `0.823083/0.37873` 回代验证（= `116.3099°E,39.9584°N`，与中关村南大街 5 号差 ~80 m），
  再去瓦片上**逐个按图索骥**确认落点有校区：
  - 良乡 `116.1666°E, 39.7290°N`、中关村 `116.3099°E, 39.9584°N`（沿用原常量）
  - 珠海 `113.5370°E, 22.3708°N` —— 图上就是「北京理工大学（珠海）」校区正中
    （周围是外国语/商学院/航空学院等学院楼、文体中心、读书岛、学生宿舍 25–40 栋）
  - ⚠️ **西山实验区未收录**：它是国防实验区，公开地图不给校区多边形（在瓦片上查过
    冷泉东路一带，只有居民区/公交首末站），加了只会跳到一片普通居民区
- 探测用的工具落在 `.workbuddy/archive/tools/map_tile_probe.py`（拼接瓦片 + 画十字标记 + 经纬度反查）
- 单测：新增 `MapCampusTest` 5 条 —— 把每个校区**反算回经纬度**断言落在期望城市、
  与标注值差 < 500 m，另加往返自洽与「按钮单字唯一」。这类常量抄错一位就会把地图带到几十公里外，
  而纯 UI 行为单测覆盖不到，所以专门锁一道
- 单测合计 **462 全绿**（新增 map 6）

## 2026-09-26 v1.9.11 空教室三级折叠 + 一卡通余额趋势 + 两处小修

- **空教室改成「校区 → 教学楼 → 教室」三级折叠**（用户要求）
  - 原来只列**当前校区**的教学楼。现在一次拉全部校区的楼（`getBuildingInfos()` 不带
    `campusCode` 时 SDK 不加校区过滤参数），客户端按 `XXXQDM` / `XXXQDM_DISPLAY` 分组
    （纯逻辑 `CampusGrouping`，6 条单测）
  - 设置里的「当前校区」改名 **「默认校区」**：它现在只决定「哪个校区排最前 + 进来自动展开」
  - ⚠️ 拉全部失败/返回空时**自动退回「只取默认校区」**（= 改版前的老行为），不会比原来更差
  - 布局不新增主题色：校区层用中性 `surfaceContainerHigh`，避免与右下角 FAB、
    教室卡片糊在一起（那是原实现踩过的坑）
- **一卡通余额趋势**（用户要求，「用余额差替代拿不到的流水」）
  - 每次成功取到余额就往 `filesDir/campus_card_balance_history` 记一笔采样
    （纯文本 `时刻|金额`，可直接 `adb shell cat` 看）
  - 详情页显示 **`今日 -¥5.95 · 近 7 天 -¥25.95`**；正数是充值、负数是消费
  - 口径诚实优先：今天只采到一次就说不出「今日变化」（不给错数）；历史不足 7 天时
    如实写「近 3 天」而不是硬说 7 天；窗口内没有采样就只报今日
  - 同一天、金额没变的连续采样**合并**（保留最早那条），文件不会被无效重复撑大
  - 一条历史都没有时显示「记录中（攒够历史后显示变化）」而不是整行消失
  - 纯逻辑 `CampusCardBalanceLogic`（18 条单测）
- **修：桌面组件点课表条目没跳到课表 tab**
  - `WidgetLine.openTab` 为 null 时，`GotoRequest.Focus(tab=null, key=null)` 会被 `takeIf`
    判成「无附加信息」丢掉 → `TabPager` 拿到 `requestedPage = null` 就不切页。
    App 已经停在课表页的 DDL / 动态 tab 上时，点课程行就会「毫无反应」
  - 现在课程行与**空闲行**都显式带 `openTab = ScheduleTabs.COURSE`（空闲行也一并变可点：
    整页语义都是「进 App 看课表」，只有课程行能点会让人觉得空档处坏了）
- **修：一卡通详情页余额少了 `¥`**（服务端给的是裸数字 `34.05`）
- 单测：`data` 154（+18）/ `features:schedule` 12（+6）/ `features:widget` 69 / 合计 **456 全绿**
...
本轮从 BIT101 网页版逆向出新契约并接上，真机实测**已同步 50 门课**，链路复活。

- **新的成绩链路**（`ScoreQueryApiService`，挂在认证主机 `login.bit101.flwfdd.xyz`）：
  `POST /api/jwb/bit101/score {username,password}` → `202` challenge_id + access_token
  → `GET /api/auth/{id}`（头 `X-Challenge-Token`）轮询到 `ready_services` 含 `jwb`
  → 带 `Authorization: Bearer` 再请求原路径，`data` 是「第 0 行表头」的二维数组
- ⚠️ **加了 12h 限频**（`ScoreQueryLogic.CHECK_INTERVAL_MS`）：新流程每次检查都要用账密
  走一遍**学校统一身份认证**，而检查触发点是「App 启动 + 每日任务」——不限频等于每次开
  App 登录一次，必撞学校风控（2026-09-26 实测：短时间多次登录 → `waiting_sms`）
- ⚠️ **修掉一个真 bug（课程列定位）**：真实表头 `['序号','开课学期','课程编号','课程名称','成绩',…]`
  里 `课程编号` 在 `课程名称` **之前**，而原实现 `contains("课程")` 取第一个 → 会拿课程编号
  （`09000410`）当课名。改成**关键词优先级 + 排除词**，并用真实表头锁进单测
- **状态可见**（`ScoreCheckStore` + 设置页）：成绩检查受学校风控影响且是后台静默跑的，
  设置 → 提醒 里显示「最近检查：MM-dd HH:mm · 已同步 N 门课 / 学校要求短信验证，已暂停 /
  获取失败 / 未登录」，并新增**「立即检查一次」**按钮（跳过限频，用户手动确认用）
- 跳过限频时**不覆盖已有状态**（原来启动时会被调用两次，第二次把真实结果盖成「刚检查过」）
- 顺手：出分提醒开关的副标题改为「点开通知直接进成绩页」（v1.9.8 已改深链）
- 单测：`ScoreLogicTest` 16（含真实表头）、`ScoreQueryLogicTest` 3、`ScoreCheckStoreTest` 3
- 真机验证：设置 → 提醒 → 立即检查一次 → 「已同步 50 门课」（**未触发短信验证**）
...
- **新增流量提醒**（用户明确要求）：本月用量到 **270 GB 提醒一次**、到 **300 GB（限速阈值）
  提醒一次**，之后不再打扰；跨计费周期（用量归零）**自动复位**，下个月才会再提醒。
  阈值硬编码 —— 套餐不变就不动（`NetFlowChecker`，纯逻辑 `decide()` 有 4 条单测）
- **口径修正**（用户确认官方门户就是按月显示）：
  - 详情页「已用流量」→「**本月已用流量**」
  - 详情页「累计在线时长」→「**本月在线时长**」（计时**按设备**，多设备会各记一份，
    所以可能超过一个月）
- 站内：`NetFeeCheckWorker` 取**一次**数据给余额与流量两个检查共用；删掉已无调用者的
  `NetFeeChecker.checkAndNotify`；新渠道 `netflow_reminder`（与「网费不足」分开，名字才说得清）
- ⚠️ 实测截图：用户本月已用 **366.4 GB**，**已超 300 GB** → 功能上线即触发，正好验证整条链路

## 2026-09-25 v1.9.6 校园服务「失败 / 未登录 / 未认证」三种状态分开

- 校园网返回 `CampusNetResult`（Online / NotOnline / Failed）替代可空值：原来三种情况
  塌成一个 `null`，UI 一律说「需连接校园网」—— 用户明明在校内也会被这么告知
  （接口无在线会话时返回 `not_online`），完全无从下手
- 详情页未认证时给「**去认证**」按钮（`isInternalUrl` 放行 `10.0.0.55`，留在 App 内 WebView）
- 一卡通同类问题一并修：请求失败原来被当成「未登录」（误导用户白登一次）→
  `CampusCardSnapshot.failed` 区分，失败时给「重试」文案且不显示登录按钮
- 单测 +3（共 163 条）

## 2026-09-26 v1.9.5 网费提醒真正可用 + WebView 主线程卡顿修复

- **网费提醒以前等于没做**：深澜接口只有校园网可达，而每日周期任务的触发时刻是固定的
  （约等于首次启动 App 的时刻）—— 那个点人在校外就会被**永远跳过**。
  现在补上「拿到数据就立刻判」这条路径：App 前台取到校园网数据 = 人在校内，
  是唯一可靠的判断时机（`NetFeeChecker.check(info)`，仍按天去重，不会刷屏）。
  另外给 Worker 补了 `NotifySentStore.init`（去重表未初始化时读写会失败）
- **WebView 每次导航都主线程卡一下**：`doUpdateVisitedHistory`（主线程回调）原来**无条件**
  `runBlocking` 读两次 DataStore 的学号/密码（磁盘 IO）→ 改为先按 URL 短路，再放进协程读
- 两处均由只读代码复核独立发现（`WebScreen.kt:153-159`、`NetFeeCheckWorker` 可用性）

## 2026-09-25 v1.9.4 修「一卡通登录后无法刷新」+ DDL 变动即时排提醒 + 工程整理

- **详情页终于能刷新**：v1.9.2 加的「登录一卡通」按钮点完回来，页面**没有任何刷新入口**
  （无下拉、无按钮），余额永远停在「未登录」—— 注释承诺的「下拉刷新」根本不存在。
  现在：顶栏加刷新按钮 + 从 WebView 返回时自动重刷（首次进入被 `_loading` 守卫挡掉，不重复请求）
- **文案**：「点开流水」→「点开详情」（流水功能并不存在，别承诺）；详情页内的未登录文案
  改「未登录，点下方「登录一卡通」」（原来复用卡片的「进详情页登录」，人在详情页里很怪）
- **校园网详情**补「本次收发 ↓/↑」（`bytesIn`/`bytesOut` 早就解析了但没展示）
- **DDL / 作业变动立刻重排提醒**：`NotifyAppStartup.reschedule()` 注释写着「课表同步完成、
  DDL 增删改后调用」，实际只在座位预约和设置开关处调过 —— 新加的 DDL 要等下次冷启动或
  每日周期任务才排上，几小时内到期的会漏提醒。现已在 DDL 增/删/改/勾选、延河课堂同步
  （DDL 页与 DDL 设置页两条路径）全部接上
- **工程**：调试探针从 `api` 主源码集迁到独立源码集 `src/probe`（release 的
  `minifyEnabled=false`，探针会连 `main()` 一起打进发布包）；`:api` 主源码集不再依赖
  coroutines（那是探针带进来的）；`api/sms.txt` 进 .gitignore；
  `WebScreen.isInternalUrl` 加固（域名大小写、URI 解析异常兜底、外跳 startActivity 包 runCatching）
- `settings.gradle` 依赖源前置阿里云镜像（直连大 jar 实测 ~14KB/s 会卡死构建，镜像 ~1.8MB/s）

## 2026-09-25 v1.9.3 修「学校登录页被踢到外部浏览器」（一卡通登录白登）

- `features/web` 的 `shouldOverrideUrlLoading` 只认 bit101.cn，**任何**非本校
  App 域名都被丢给系统浏览器 → 一卡通 CAS 登录在 App 外完成，cookie 从不落到
  App 的 CookieManager，卡片永远显示未登录
- 修复：**学校域名（`*.bit.edu.cn`）一律留在 App 内 WebView**（与座位/eclass
  登录同一条规矩）；外部站点仍交系统浏览器
- 影响面：一卡通登录、DDL 页的延河课堂登录、动态页设置里的乐学入口都受益
...
- **校园网（深澜 Srun）免登录接口接入**：`10.0.0.55/cgi-bin/rad_user_info`
  实测免登录返回在线数据（仅校园网环境可达，校外静默降级提示）；
  字段映射经两样本对照锁定（账号/上线时间/收发字节/累计流量/累计时长/IP/余额）
- 「校园服务」详情页（原生，不再跳网页）：一卡通余额 + 校园网
  账号/已用流量/累计时长/本次上线/IP/余额；我页两张卡常显摘要、点开进详情
- **网费不足提醒**：每日 Worker 检查，余额 <10 元发通知（新渠道
  `netfee_reminder`，按天去重，跳「我」页；不在校内静默跳过）
- 一卡通**流水页暂缓**：`/myaccount` 重定向到独立 cardpay 登录（非 CAS），
  等摸清免登通道（或用户提供查询密码）再接入
- 协议说明：上游为 **AGPL-3.0**（非 MIT），fork 已同协议开源

## 2026-09-24 v1.8.1 修「一卡通登录后仍显示未登录」+ 校园网直达登录页

- 🐛 **一卡通卡片永远显示「点开登录后显示」**：登录判定原来按 HTML 关键词
  （页面含「统一身份认证」等就当登录页），而一卡通首页正文/菜单里就有这类文案
  → 已登录也被误判。改为**看重定向之后的最终 URL**（未登录时服务端 302 到
  CAS，最终主机是 sso.bit.edu.cn）—— 唯一可靠的信号；解析抽成纯函数
  `CampusCardLogic`（6 条单测，含该 bug 的回归用例）
- **校园网卡直达登录页**（`/client/signIn`）：校园网是深澜（Srun）缴费系统，
  SPA 有独立的账号登录页；之前开的 `entry=dingtalk` 是 i北理（钉钉壳）免登入口，
  在普通 WebView 里渲染成空壳（就是「没什么用」的原因）
- ⚠️ Gradle 缓存被环境反复损坏（`transforms/*/metadata.bin` 被外部进程删除），
  已全量重建；并在 `~/.gradle/gradle.properties` 配置了 Clash 代理
  （github.com 直连超时，kotlin-compiler 等依赖需要走代理）

## 2026-09-24 v1.8.0 「校园服务」卡片（我页）：一卡通余额摘要 + 校园网入口

- 「我」页顶部新增两张卡片（仅本人页显示）：
  - **一卡通**：余额摘要（启发式解析首页 HTML）+ 点开 App 内 WebView 看流水。
    一卡通走学校统一身份认证（USTC CAS，与座位系统同构）——在 App 内 WebView
    登录一次，cookie 与快照请求共享
  - **校园网**（`netpay.bit.edu.cn:8091`）：接口形态未确认（i北理里疑似只认
    钉钉免登），首版只给 WebView 入口，抓包确认后升级原生
- 数据层：`CampusCardRepo`（首页快照 + 启发式余额解析 + 调试日志）。
  余额解析依赖首页真实形态，**首次真机使用请在 logcat 看 `CampusCardRepo` 标签**，
  据此写精确解析（v1.8.x）

## 2026-09-23 v1.7.8 修「动态页误报未登录」+ 动态页下拉刷新 + DDL/动态设置页补动作项

- 🐛 **动态页有时显示「未登录」、设置页却显示已登录**：`isSessionAlive()` 是一次
  真实网络请求，超时/5xx/校园网抖动都会让它失败，而失败被当成「未登录」缓存住、
  不再复查，重启才恢复。修复：① 检查前先把 WebView cookie 同步过来（此前只有
  取数路径做同步，检查路径不做，两处判据不一致）；② 失败重试一次，两次都失败才报未登录
- 「动态」页支持**下拉刷新**（与座位页任务列表同一套 material3 1.2 组合）
- 「动态设置」页新增「数据设置 → 重新拉取动态」：经跨页刷新总线（`EclassRefreshBus`）
  通知活着的动态页立即重新取数，并顺手复查会话状态
- 「DDL 设置」页新增「延河课堂」组：**重新拉取延河课堂作业**（已完成状态保留）+
  会话状态 / 打开登录页 —— 新旧两个来源的动作项并列
- 延河课堂作业同步逻辑抽成单例 `EclassDdlSyncer`：DDL 页与 DDL 设置页共用同一份
  「不存在插入 / 存在更新且保留 done」的合并规则，不再各写一份

## 2026-09-23 v1.7.7 手动修改课程表（覆盖层，同步不冲掉）

设置 → 「课程编辑」：对教务拉来的课表做**编辑 / 隐藏 / 新增**，改动存独立
`course_overlay` 表（数据库 v2→v3 自动迁移），**不改原表**——重新同步教务
课表时覆盖层原样保留，用户的手工修正不会被冲掉。

- 三种覆盖：EDIT（改课名/教师/教室/周次/时间）、HIDE（藏起来）、ADD（本地补课）；
  课表渲染时合并（`CourseOverlayLogic.merge`，纯函数 17 单测）
- ⚠️ **锚点模型**：EDIT 的匹配键（学期+课程号+星期+开始节次）既是键又是显示值，
  「改上课时间」必须把**锚点**（指向教务原课）与**显示值**分开——
  锚点在数据层锁死（`normalizeAnchors`：更新时沿用 existing、ADD 自锚），
  UI 二次编辑传漂移锚点也伤不到数据。这是本轮最重要的设计修正
- 编辑/隐藏/恢复/新增界面（`CourseEditPage` + `CourseEditViewModel`，35 单测）
- 兼容：`getCoursesFromLocal` 三个重载全部走合并后的结果，所有消费方
  （桌面组件课程页、DDL、提醒）自动生效
