...
## 2026-09-27 v1.9.13 地图页补上嘉兴校区（附「查不到坐标时怎么办」的可复现方法）

- **地图页新增第四颗按钮「嘉」= 嘉兴校区**，与乡/村/珠 一致：点击定位 + 弹「已切换到嘉兴校区」+ 按钮高亮。
  坐标 `120.7399°E, 30.8326°N`（→ `x=0.835389, y=0.409893`）。
- **坐标取的是「院区中心」，不是那个 POI 点**（这点踩过）：
  百度 POI「北京理工大学长三角研究院」标在**院区东侧边缘**，直接用会偏出约 240 m；
  院区一期 500 亩、建筑群实测跨度约 570 m，最终坐标是在百度**卫星图**上量出建筑群
  范围后取的中心。核对到的第三方信息：官方地址**东方北路 1940 号**（教育部批文，与
  百科一致）、院区在**银杏天鹅湖**畔、一期南区块已投用、紧邻杭州师范大学附属油车港中学西侧，
  并且配套「改造提升马厍老街」。
  ⚠️ **我们那套瓦片底图比百度旧**（校园 2025 年才交付），所以地图页跳过去看到的是农田与鱼塘
  ——**这不是坐标错，是底图数据早于校园建成**。
- **这个坐标来得不容易，过程值得记下来**（嘉兴是新校区，网上只有文字地址）：
  - 招生章程/新闻只给地址「浙江省嘉兴市秀洲区油车港镇东方北路 1940 号」——**没有任何坐标**
  - OSM / Overpass / Nominatim 里**没有这所学校**（嘉兴其他高校都在，唯独它没有），
    连「北理工长三角研究院」公交站都没收录
  - 高德 `ditu.amap.com` 的 POI 接口是阿里反爬（punish 页，带 Cookie/Referer 也不行）；
    百度 `map.baidu.com/?qt=s` 要验证码，`qt=inf` 也是
  - 瓦片底图上**校园还没画出来**（2025 年才投用，底图数据早于建成），扫遍油车港镇/塘汇/王江泾
    一带十余张拼接图，只有村庄、鱼塘、莲泗荡、南官荡
- **最后走通的路：借百度地图 POI 的「分享链接」让百度自己把坐标说出来**
  1. 手机百度地图搜到该地点 → 分享 → 复制链接，形如
     `https://map.baidu.com/?newmap=1&s=inf%26uid%3D<uid>&sharecallbackflag=poiDetailPage`
  2. 用无头 Chromium 打开它，等 SPA 渲染完，**读 `location.href`** —— 百度会把自己算好的坐标
     写进地址栏：`.../poi/北京理工大学长三角研究院/@13442229.78,3589392.66,19z?uid=...`；
     页面自身的 `qt=detailConInfo` 响应里也有 `"geo":"1|13442329.28"` / `"x"/"y"`
  3. 该值是 **BD09-MC**，依次 BD09-MC → BD09 → GCJ02 → **WGS84**（瓦片用的是 WGS84，
     直接把 BD09 当 WGS84 用会**偏出约 1 km**）
- ⚠️ **两个坑**（都踩过，写进 `MapCampus` 注释了）：
  - `s=inf&wd=<名称>`（按名称查）会**静默回落到北京视角**，看着像成功、其实给的是默认坐标；
    **只有带 `uid` 的分享链接可靠**
  - 在页面内 `fetch('?qt=inf&uid=...')` 会**挂住**（页面自己的请求被反爬参数保护），
    别走那条路，读地址栏即可
- **真机验收**：图页出现 乡/村/珠/**嘉** 四颗按钮，默认高亮「乡」落在良乡校区；点「嘉」→
  顶部弹「已切换到嘉兴校区」、按钮高亮，地图落在**油车港镇西侧**——图上可见
  **「孔家港」「罗家甸」「荡西」「吴家湾」「油车港镇实验小学」**与那条主干道（油黎公路）。
  其中「孔家港」正是用户截图里 POI 上方的同一地名，方位一致 ✓
- 单测：`MapCampusTest` 的期望表与城市范围表各补一条（共 **462 条全绿**，map 模块 6 条）。
  ⚠️ 另更正一处旧记录：v1.9.12 的 `versionCode` 是 **44**（不是 43）。
