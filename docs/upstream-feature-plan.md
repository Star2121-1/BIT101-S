...
> 整理时间：2026-09-20
> 状态：**分支 A（`feature/widget`）已完成（v1.6.0，后续迭代至 v1.6.7）；
> 分支 B–D 未开工，优先级已重排 —— 见 `docs/next-modules.md`（2026-09-22 校准）**
...
> ⚠️ **2026-09-22 更新**：本表已被 `docs/next-modules.md` 的「建议执行顺序（更新版）」取代。
> 变化：用户点名的**通知与提醒中心**、**DDL 换源**（学校已弃用乐学）、**校园卡 / 消费**
> 三项新模块排在前面；本表的 B/C/D 顺延。下表的「已完成」状态仍然有效。

| 顺序 | 分支 | 理由 |
|------|------|------|
| 1 | `feature/widget` | —— ✅ **已完成**（2026-09-20，v1.6.0；后续迭代到 v1.6.7：可滚动全天行程、空闲合并、跨天切换、点课程开课表） |
| 2 | `feature/schedule-edit` | 范围已收窄为「手动改课程」，核心是覆盖层设计（**顺延**，见 next-modules.md） |
| 3 | `feature/security` | 收益大但风险最高（Cookie 迁移可致全体掉登录态），放后面有回退空间 |
| 4 | `feature/map-navigation` | 独立栈、难度最高，最后单独攻 |
| — | ~~「手动添加日程」~~ | **上游已实现，我们已继承，无需再做** |
| — | ~~NFC~~ | **放弃**（见 3.1） |
| — | ~~空教室查询~~ | **已有**（课表页第二个 tab）—— 用户 2026-09-22 指出，核对属实 |
| — | ~~组件显示成绩~~ | **不做**（隐私考虑；App「网」页已有成绩查询） |
