...
每页最多 **3 行**，但实际行数**跟着组件高度变**（见下文「高度可拖矮」）；
4×3 全高时字号为主行 16sp / 次行 14sp。

一行内容的结构是 `[节次] 课程名 | 上课时间 | 教室`，时间单独占一列 ——
塞进课程名里会被省略号吃掉。
...
| 情况 | 表现 |
|------|------|
| 正在上课 | 行首加 `▶`，课程名与时间用**绿色**加粗 |
| 课间（下一节还没开始） | 下一节同样加 `▶`，用**橙色**加粗 |
| 今天的课都上完了 | 不高亮，改为显示今天最后几节 |

⚠️ **显示窗口也跟着走**：一天上到第 6 节时，组件不会还停在上午的前三节 ——
窗口从「正在上 / 下一节」开始往后排（见 `WidgetLogic.coursePage`）。
组件被拖矮只显示 1 行时，这一行仍然是高亮的那节。
...
组件纵向可缩到 **110dp（4×2，约全高的三分之一）**，行数自动降档：

| 上报高度 | 行数 |
|------|------|
| ≥ 140dp（4×3 全高） | 3 行 |
| ≥ 100dp（含最小可缩高度 110dp） | 2 行 |
| 更矮 | 1 行 |

阈值由实际布局算出（上下内边距 20 + 页签 36 + 内容上边距 6 + 每行约 20dp + 行距 6dp）：
3 行需 ≈ 134dp、2 行 ≈ 106dp。判定逻辑是纯函数 `WidgetLogic.rowsForHeight()`，有单测覆盖。

⚠️ **读的是 `OPTION_APPWIDGET_MIN_HEIGHT`，而不是 MAX** —— 这点实测过，别想当然：

| 量 | 实测值（Pixel 6 API 34 + Pixel Launcher） |
|---|---|
| 4×3 组件在屏上的**实际**高度 | **343dp** |
| 系统上报 `MIN_HEIGHT` | 193dp |
| 系统上报 `MAX_HEIGHT` | **342dp** ← 才等于实际高度 |

即 **MAX 才是当前高度，MIN 是「能拖到多小」那一侧的下界**。仍然取 MIN 是因为它
**保守**：报小了最多少显示一行，报大了会让 3 行挤进矮组件、上下一起被裁
（内容区是 `weight=1 + center_vertical`，溢出时**首行也会受损**）。
而 4×3 下 193 与 342 都远超 140dp 阈值，结果一致；拖矮时 MIN 随之下降，正好触发降档。

⚠️ **数据与布局必须用同一个行数上限**：`BIT101WidgetProvider.render()` 算一次
`limit`，既用来取数据（`WidgetRepositoryHolder.load(context, limit)`）也用来建视图
（`WidgetViews.build(..., rowLimit = limit)`）。只用其中一边会出现
「取了 3 行只画 2 行」的浪费，或反过来把没取到的行画成空白。

拿不到尺寸信息（部分 ROM 不写 `options`）时按完整行数渲染 —— 宁可多画一点被裁，
也不要无端少显示信息。

### 内容垂直居中（为什么内容区套了一层）

组件默认 4×3（180dp 名义高度、实际 343dp），而 3 行内容只有约 78dp。
若直接顶部对齐，**底部会留一大片空白，看起来像没渲染完**（v1.6.2 截图里发现）。

所以 `widget_root.xml` 里内容区是一层 `layout_height=0dp + layout_weight=1 +
gravity=center_vertical` 的容器，把「空态文案 + 内容行 + 立即预约按钮」整体居中。
这样无论用户把组件拖成什么高度，内容都居中，不会挤在一角。

### 座位页的「立即预约」

座位页只要有**空余行位**，底部就多一个「立即预约」按钮，点了打开 App 并
**直接落在座位页**（不是只把 App 唤到前台）。

| 情况 | 按钮 | 空态文字 |
|------|------|---------|
| 空态 + 全高 | 显示 | 显示 |
| 空态 + 拖矮（2 行以下） | 显示 | **隐藏**（让位给按钮） |
| 有 1–2 行内容 | 显示 | — |
| 内容占满 3 行 | 隐藏（会被裁掉半截，不如不显示） | — |
...
（纯函数，`now` / `today` 显式传入）。单测 `WidgetLogicTest` **32 条**。
...
- `coursePage` 的**起点**分三种情况（有焦点从焦点开始、时间未知从头、
  今天上完取末尾），三者容易混成一种，所以分开测
- `rowsForHeight` —— 三档阈值 + 「系统没给尺寸时按完整行数」+ `full` 上限
- `courseTimeText` —— 节次换算成时间、越界时返回空串而不是编一个

⚠️ 一个注意点：**单测里不要图省事合并「时间未知」与「今天上完」两条路径。**
v1.6.2 开发时就因为把两者合并成「一律取末尾」，导致降级路径下显示当天最后几节课，
是被 `次行最多两条` 这条老测试拦下来的。
