...
**四页**（课程 / DDL / 动态 / 座位），**点顶部页签直接切换**，右侧有「刷新」键。
内容区是**可上下滑动的列表**。⚠️ 页序与**页号存的是页名**（`WidgetPageStore` 存
`PageKind.name`，不是下标）—— 重排页序不会让老用户的页号指错页（历史 int 按旧顺序表还原，幂等）：

| 页 | 内容 | 空态文案 | 点条目 |
|---|------|---------|--------|
| 课程 | 当天完整行程：每节课 + 合并后的空闲时段，按节次升序 | 今日无课 | 进 App 并停在**课表 tab**（课程行与**空闲行都可点**） |
| DDL | 全部 DDL（不限时），分「未完成 / 已完成」两栏 | 暂无待办 | 进 App 的 **DDL tab** 并定位到该条（定位键 `ddl:{uid}`） |
| 动态 | 延河课堂的课程动态（近若干条，按设置过滤） | 暂无动态 | 进 App 的**动态 tab** 并定位到该条（`eclass:{id}`） |
| 座位 | 进行中的抢座任务 → 补已预约的记录 | 暂无预约 | 不可点（动作键「一键/立即预约」另有入口） |

⚠️ **行点击 = `openRoute` + `openTab` 一起给，`openTab` 不能省**（2026-09-26 修）：
`GotoRequest.Focus(tab = null, key = null)` 会被 `takeIf` 判成「没有附加信息」而丢掉，
`TabPager` 收到 `requestedPage = null` 就**不切页** —— App 已经停在课表页的
DDL / 动态 tab 上时，点课程行会「毫无反应」。
...
改成**页签**后：每页签宽 ≈ (组件宽 − 48dp) / **页数**、高 36dp，触摸面积是原来的
...
- [x] 模拟器：点「刷新」无崩溃
- [x] 真机（NP05J）：页签切换、刷新、**切回课程页**全部即时生效（当时三页，现为四页）
- [x] 真机：课程页显示真实课表（含教室号）
- [ ] 真机：厂商 ROM 对 WorkManager 的限流
- [ ] 真机：跨零点自动换天
- [ ] 真机：App 内同步课表后组件内容更新
