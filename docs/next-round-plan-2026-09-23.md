...
## 四、决策结果（2026-09-23 全部落定，v1.7.6 已实现）

1. **动态页设置项**：只看作业**默认关**、条数上限 60（30/60/100）、显示已过期默认开 +
   延河课堂/乐学两个入口 —— 已实现
2. **W5 精确定位：做了**（定位键自描述 `ddl:`/`activity:` → App 列表滚到那一条）
3. **「勾选模式」不做**（用户 2026-09-23 决定）→ 相关字段（`toggleDdlUid`）、模板
   （`ddlToggleTemplate`）、广播（`ACTION_ITEM_TAP`）与 `toggleDdlDone` **已全部删除**，
   不留死代码。⚠️ 硬限制记在这里，别再尝试：
   **RemoteViews 没有长按 API**，且**一个 collection 只能挂一个点击模板** ——
   组件内的「勾选」在架构上做不到
4. **乐学动态汇总**：先上 C（设置页里的 WebView 入口，已做）；
   B（抓包调研乐学的公告/动态出口）**待用户配合登录一次**（我方对乐学只有 ICS 日历）
