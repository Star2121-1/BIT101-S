...
### ⚠️⚠️ 渲染规则：五张图**不是互补图层**（改动前必读，踩过两次致命 bug）

**这五张图各自都是「一张完整的房间图」** —— 同一位置在不同图里画不同图标
（free=绿座位+号、book=文件、use=人、leave=时钟、close=黄座位）。

正确做法：**每个座位按自己的 status 挑一张图、只取那一小块**，逐座位拼出整张画面。
官方 `chunk_seat-map.js` 的 `S` 函数就是这么做的（逐座位一个 div + `background-image`
+ `background-position: -x,-y`）。

#### ❌ 绝不能把五张图叠起来（v1.6.0 前的实现，已推翻）

后叠的会盖住先叠的，而 `leave` 恰好排在最后 → **所有座位都显示成「临时离开」的时钟**。
用户 2026-09-20 实测报出「视听学习空间所有座位都不对」。

#### ❌ 绝不能给每个座位一个 `AsyncImage`（会 ANR）

103 个座位的区域会造出 100+ 节点 + 100+ 请求 + 100+ 份 8MB 位图，
主线程卡死 5 秒触发 **ANR**（2026-09-20 实际炸过）。

#### ✅ 现行实现（2026-09-20 最终版，勿回退）

1. **按 status 去重**，最多加载 5 张 `ImageBitmap`
   （`coil.imageLoader.execute` → `toBitmap().asImageBitmap()`，`Dispatchers.IO` + `produceState`）
2. 在**一个 `Canvas`** 上逐座位绘制，用 **`drawImage` 的 `srcOffset` / `srcSize` 做源图裁剪**：
   `srcOffset = (x, y)`、`srcSize = (w, h)`、`dstOffset = (x, y)`、`dstSize = (w, h)`
   —— 数学上等价于官方的 `background-position: -px -py`
3. ⚠️ 位图未就绪时**整块不画**（「全有或全无」）：否则第一帧 103 个瓦片全部 return，
   只剩 `free` 底图 = 半成品「全绿画面」（看起来所有座位都可用）

#### ❌ 绝不要用 `withTransform { translate(-x,-y) }` + `clipRect(x,y,x+w,y+h)`

曾这么做，结果是**覆盖层完全不可见**：`withTransform` 内的 `clipRect` **也会被同一个
translate 变换**，实际裁的是画布 `(0,0,w,h)`，而图已经平移到 `(-x,-y)` 起画 ——
两者错开，瓦片被整个裁没。因为画布左上角在视口外（初始缩放 1.8、原点为负），
连错位的痕迹都看不到，表现为**露出 `free` 的「全绿底版」**。

#### ✅ 验收硬标准：逐像素差分，不要只靠肉眼看截图

用 Python + Pillow 按官方 `S` 函数公式 + 真实坐标**重绘基准图**，与 App 截图逐像素比对。

2026-09-20 实测：**平均差 0.0211、差 >0.40 仅占 1.16%**（残差为 JPEG 压缩伪影）
= 与官网渲染一致；「在用」蓝灰图标的像素数 675 → **4645**（基准 4547）。

⚠️ 对齐基准图时的几何要点：`INITIAL_SCALE = 1.8` 下图片显示为 1944×1093.5、`offsetX = -432`，
**App 看到的是完整的一整张 16:9 图（纵向没有裁），只有左右各裁 432px** ——
所以是「整图放大到 1944 再水平裁 [432, 1512]」，**不是**「中心裁 1/1.8」（后者差 0.2851）。
工具见 `.workbuddy/archive/tools/`（`baseline_render.py` / `diff_final2.py`）。
